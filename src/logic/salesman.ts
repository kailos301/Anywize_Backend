 ...
 * @private
 */
 function Path(points, calculateDistances = true) {
  this.points = points;
...
Path.prototype.inputDistances = function (mapboxDistanceMatrix) {
  this.distances = new Array(this.points.length * this.points.length);
  for (var i = 0; i < this.points.length; i++)
    for (var j = 0; j < this.points.length; j++)
      this.distances[j + i * this.points.length] = mapboxDistanceMatrix[i][j];
};
...
function solveWithManualDistances(points, mapboxDistanceMatrix) {
  const path = new Path(points, false);

  if (points.length < 2) return path.order; // There is nothing to optimize

  const temp_coeff = 1 - Math.exp(-10 - Math.min(points.length, 1e6) / 1e5);

  path.inputDistances(mapboxDistanceMatrix);
